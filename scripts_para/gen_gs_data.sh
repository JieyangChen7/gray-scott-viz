...
DATA_DIR=/gpfs/alpine/scratch/jieyang/csc331


GSBIN=$CODE_DIR/gray-scott/build/gray-scott

VTKH_RENDER=$CODE_DIR/vtkh/build/gray-scott-vtkh
PROFILER_PREFIX=$CODE_DIR/scripts/compressor_profiler
...
gen_gs_data() {
    ...
    jsrun --nrs 1 --tasks_per_rs 125 --cpu_per_rs 42 --rs_per_host 1 --latency_priority CPU-CPU --launch_distribution packed --bind packed:1 $GSBIN ./${_BPNAME}.json ${_START_ITER} ${_COMPRESSOR_U} ${_TOL_U} ${_COMPRESSOR_V} ${_TOL_V}
    #$GSBIN ./${_BPNAME}.json ${_START_ITER} ${_COMPRESSOR_U} ${_TOL_U} ${_COMPRESSOR_V} ${_TOL_V}
    cd ${CODE_DIR}/scripts
}
...
TOTAL_STEP=600
STEP_GAP=30
TOTAL_ITER=$((${TOTAL_STEP}/${STEP_GAP}))
START_STEP=0

echo 'generating Gray-scott simulation data'
gen_gs_data ${SIZE} ${NOISE} ${TOTAL_STEP} ${STEP_GAP} ${START_STEP} ${COMP_U} ${TOL_U} ${COMP_V} ${TOL_V}
